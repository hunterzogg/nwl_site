...
**Cache-busting:** `style.css` is loaded as `style.css?v=8` from every page (currently) — bump this query string (find/replace across all HTML files) any time style.css changes, since GitHub Pages / most browsers cache it aggressively and won't otherwise pick up edits. Same idea applies to `shared.js?v=14` (bumped this session when `shared.js` changed for the nav update below — same browser-caching reason, confirmed via a real repro: after editing `shared.js` without bumping the version, a fresh page load still showed the pre-edit nav until the version query string changed) if that file is ever changed again, and to `pickem.js?v=5` on `pickem.html` specifically. **Caught the gotcha in the wild again this session**: edited `style.css` for the banner-narrowing pass below, verified in-browser, and the padding change silently didn't apply — turned out to be this exact caching issue since `v=7` hadn't been bumped yet. Always bump immediately after any `style.css` edit, before verifying.

**Gemini-proposed "Modern Design System" evaluated, partially adopted (added this session).** Hunter pasted a full redesign spec from Gemini (new color tokens, JetBrains Mono, system-font headers, glassmorphism, pill badges, horizontal-scroll+sticky-column tables) and asked for a review before touching anything. Recap given: it's a generic sports-dashboard template (placeholder team names, no awareness of this site's actual content model), and two of its pieces directly conflict with deliberate, documented choices — the Oswald-driven uppercase/condensed header identity, and the `.table-wide`/`.table-cards` responsive pattern immediately above this note. Per explicit instruction, only the safe/additive pieces were adopted, typography and tables were left untouched:
- **Card hover-lift**: new shared `.hover-lift` utility in `style.css` (`transform: translateY(-2px)` + `border-color`/`background-color` swap to new `--line-strong`/`--surface-raised` tokens on hover), guarded behind `@media (hover: hover)` (no stuck hover state on tap) and `@media (prefers-reduced-motion: reduce)`. Applied to `season-2026.html`'s `.matchup-card`/`.rank-card`/`.fact-card` and (self-contained, not the shared class, since `.mgr-card` already owned its own `transition` and the cascade would've dropped it - see the class-vs-cascade note in that file) `managers.html`'s `.mgr-card` (skipped on `.expanded` cards so the lift doesn't fight the persistent blue "expanded" border). Deliberately **not** applied everywhere - there are a dozen+ one-off card-like components scattered across per-page `<style>` blocks (draft.html, trade-tools.html, etc.); only the two highest-traffic card-grid pages got it, to keep this a "safe pieces" pass rather than a full redesign.
- **Live pulse animation**: `.nav-live-dot` (the small dot next to "2026 Hub" in the nav and every `.season-banner`) now pulses via a `@keyframes live-pulse` box-shadow ring, previously static.
- **Skeleton loading shimmer**: new shared `.skeleton-block`/`.skeleton-line` classes (a `transform`-animated gradient overlay, not a repainted background, to keep it cheap). Applied only to `season-2026.html`'s five data panels (Matchups/Standings/Power/Commentary/Fun Facts) as their literal initial HTML - the async `loadDataSafe()` calls replace it via `innerHTML =` once real data lands, so there's zero layout shift and no JS needed to toggle it. Not applied elsewhere yet (every other page's content is either instant client-side computation or a lower-traffic path).

**Banner narrowed (same session, explicit "takes up too much page space" feedback):** `.season-banner` (used on `season-2026.html`, `pickem.html`, and the homepage callout) padding cut `20px 24px → 12px 20px`, margin-bottom `24px → 18px`, its `<h1>`/`<h2>` given an explicit `1.3rem` size (was inheriting the browser default `~32px` h1 size - the single biggest contributor to the old height), and the label/paragraph tightened to match (`.season-banner-label` `margin-bottom: 4px → 2px`; each page's inline paragraph style shrunk from `0.9rem`/`340px` to `0.82rem`/`~300px`). Net effect: roughly a 35-40% height reduction at desktop width. All three pages' inline paragraph styles had to be hand-edited (not just the shared CSS class) since the font-size/max-width were set inline per page, not in `style.css`.
